...
public class ExternalDataSourceProperties {
    ...
    List<HikariDataSource> build(Environment environment, Callback<HikariDataSource> callback) {
        ...
        for (int index = 0; index < num; index++) {
            ...
            if (StringUtils.isEmpty(poolProperties.getDataSource().getDriverClassName())) {
                poolProperties.setDriverClassName(resolveDriverClassName(jdbcUrl));
            }
            ...
            if (StringUtils.isEmpty(ds.getConnectionTestQuery())) {
                ds.setConnectionTestQuery(DEFAULT_TEST_QUERY);
            }
    ...
    /**
     * Resolve JDBC driver class name from URL.
     * Supports multiple GaussDB URL patterns for different compatible modes:
     *   - jdbc:gaussdb://         (standard)
     *   - jdbc:opengauss://        (openGauss compatible)
     *   - jdbc:zenith://           (Zenith / GaussDB T)
     */
    static String resolveDriverClassName(String jdbcUrl) {
        ...
        if (lowerUrl.startsWith("jdbc:gaussdb://")
                || lowerUrl.startsWith("jdbc:opengauss://")
                || lowerUrl.startsWith("jdbc:zenith://")) {
