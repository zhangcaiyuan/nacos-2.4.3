...
/**
 * The gaussdb implementation of ConfigInfoMapper.
 * Supports pg / ora / mysql compatible modes.
 *
 * @author hyx
 **/

public class ConfigInfoMapperByGaussDb extends AbstractMapperByGaussDb implements ConfigInfoMapper {
    ...
    public MapperResult findConfigInfoByAppFetchRows(MapperContext context) {
        ...
        String baseSql = "SELECT id,data_id,group_id,tenant_id,app_name,content FROM config_info"
                + " WHERE tenant_id LIKE ? AND app_name= ?";
        String sql = buildWrappedPagination(baseSql, context.getStartRow(), context.getPageSize());
        return new MapperResult(sql, CollectionUtils.list(tenantId, appName));
    }
    
    @Override
    public MapperResult getTenantIdList(MapperContext context) {
        String baseSql = "SELECT tenant_id FROM config_info WHERE tenant_id != '" + NamespaceUtil.getNamespaceDefaultId()
                + "' GROUP BY tenant_id";
        String sql = buildWrappedPagination(baseSql, context.getStartRow(), context.getPageSize());
    ...
    public MapperResult getGroupIdList(MapperContext context) {
        String baseSql = "SELECT group_id FROM config_info WHERE tenant_id ='" + NamespaceUtil.getNamespaceDefaultId()
                + "' GROUP BY group_id";
        String sql = buildWrappedPagination(baseSql, context.getStartRow(), context.getPageSize());
        return new MapperResult(sql, Collections.emptyList());
    }
    ...
    public MapperResult findAllConfigKey(MapperContext context) {
        String innerSelect = "SELECT id FROM config_info WHERE tenant_id LIKE ? ORDER BY id";
        ...
        String sql = " SELECT data_id,group_id,app_name  FROM ( "
                + innerSelect + buildPagination(context.getStartRow(), context.getPageSize()) + " )"
                + " g, config_info t WHERE g.id = t.id  ";
        return new MapperResult(sql, CollectionUtils.list(context.getWhereParameter(FieldConstant.TENANT_ID)));
    }
    ...
    public MapperResult findAllConfigInfoBaseFetchRows(MapperContext context) {
        String innerSelect = "SELECT id FROM config_info ORDER BY id";
        if (GaussdbCompatibleModeUtil.isOraMode()) {
            String paginatedInner = buildWrappedPagination(innerSelect, context.getStartRow(), context.getPageSize());
            String sql = "SELECT t.id,data_id,group_id,content,md5"
                    + " FROM ( " + paginatedInner + " ) g, config_info t  WHERE g.id = t.id ";
            return new MapperResult(sql, Collections.emptyList());
        }
        String sql =
                "SELECT t.id,data_id,group_id,content,md5" + " FROM ( " + innerSelect
    ...
    public MapperResult findAllConfigInfoFragment(MapperContext context) {
        ...
        String baseSql = "SELECT id,data_id,group_id,tenant_id,app_name," + (needContent ? "content," : "")
                + "md5,gmt_modified,type,encrypted_data_key FROM config_info WHERE id > ? ORDER BY id ASC";
        String sql;
        if (GaussdbCompatibleModeUtil.isOraMode()) {
            sql = buildWrappedPagination(baseSql, context.getStartRow(), context.getPageSize());
        } else {
            sql = baseSql + buildPagination(context.getStartRow(), context.getPageSize());
        }
    ...
    public MapperResult findChangeConfigFetchRows(MapperContext context) {
        ...
        String baseSql = sqlFetchRows + where + " AND id > " + context.getWhereParameter(FieldConstant.LAST_MAX_ID)
                + " ORDER BY id ASC";
        String sql;
        if (GaussdbCompatibleModeUtil.isOraMode()) {
            sql = buildWrappedPagination(baseSql, 0, context.getPageSize());
        } else {
            sql = baseSql + buildPagination(0, context.getPageSize());
        }
    ...
    public MapperResult listGroupKeyMd5ByPageFetchRows(MapperContext context) {
        ...
        if (GaussdbCompatibleModeUtil.isOraMode()) {
            ...
            String sql = "SELECT t.id,data_id,group_id,tenant_id,app_name,md5,type,gmt_modified,encrypted_data_key FROM "
                    + "( " + paginatedInner + " ) g, config_info t WHERE g.id = t.id";
            return new MapperResult(sql, Collections.emptyList());
        }
        String sql = "SELECT t.id,data_id,group_id,tenant_id,app_name,md5,type,gmt_modified,encrypted_data_key FROM "
                + "( " + innerSelect + buildPagination(context.getStartRow(), context.getPageSize())
    ...
    public MapperResult findConfigInfoBaseLikeFetchRows(MapperContext context) {
        ...
        String baseSql = sqlFetchRows + where;
        String sql;
        if (GaussdbCompatibleModeUtil.isOraMode()) {
            sql = buildWrappedPagination(baseSql, context.getStartRow(), context.getPageSize());
        } else {
    ...
    public MapperResult findConfigInfo4PageFetchRows(MapperContext context) {
        ...
        String baseSql = sql + where.toString();
        String finalSql;
        if (GaussdbCompatibleModeUtil.isOraMode()) {
            finalSql = buildWrappedPagination(baseSql, context.getStartRow(), context.getPageSize());
        } else {
            finalSql = baseSql + buildPagination(context.getStartRow(), context.getPageSize());
        }
    ...
    public MapperResult findConfigInfoBaseByGroupFetchRows(MapperContext context) {
        String baseSql = "SELECT id,data_id,group_id,content FROM config_info WHERE group_id=? AND tenant_id=?";
        String sql;
        if (GaussdbCompatibleModeUtil.isOraMode()) {
            sql = buildWrappedPagination(baseSql, context.getStartRow(), context.getPageSize());
        } else {
    ...
    public MapperResult findConfigInfoLike4PageFetchRows(MapperContext context) {
        ...
        StringBuilder where = new StringBuilder(
                "SELECT id,data_id,group_id,tenant_id,app_name,content,encrypted_data_key,type FROM config_info WHERE ");
        ...
        String baseSql = where.toString();
        String finalSql;
        if (GaussdbCompatibleModeUtil.isOraMode()) {
            finalSql = buildWrappedPagination(baseSql, context.getStartRow(), context.getPageSize());
        } else {
            where.append(buildPagination(context.getStartRow(), context.getPageSize()));
            finalSql = where.toString();
        }
        return new MapperResult(finalSql, paramList);
    }
